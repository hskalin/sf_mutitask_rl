...
class GaussianPolicy(BaseNetwork):
    ...
    def __init__(
        ...
        self.sizes = sizes
        self.squash = squash
        self.cat_obs_hidden = cat_obs_hidden
...
class MultiheadGaussianPolicy(BaseNetwork):
    ...
    def forward_head(self, obs, idx):
        """forward single head"""
        actions, entropies, means = self.sample(obs)  # [N, H, A], [N, H, 1], [N, H, A]
...
#     print(prof1.key_averages().table(sort_by="cuda_time_total", row_limit=10))
    ...
    policy2_scripted = torch.jit.script(policy2)
