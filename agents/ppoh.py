...
class Agent(nn.Module):
    def __init__(self, primitive):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(primitive.env.num_obs, 256)),
            nn.Tanh(),
        ...
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(256, 256)),
            nn.Tanh(),
            layer_init(nn.Linear(256, primitive.feature_dim), std=0.01),
        )
        self.actor_logstd = nn.Parameter(torch.zeros(1, primitive.feature_dim))
...
class PPOHagent:
    def __init__(self, cfg):
        ...
        self.primitive.load_torch_model(
        ...
        self.agent = Agent(self.primitive).to(self.device)
    ...
    def _init_buffers(self):
        ...
        self.obs = torch.zeros(
            (
                self.agent_cfg["num_steps"],
                self.env_cfg["num_envs"],
                self.primitive.env.num_obs,
            ),
            dtype=torch.float,
        ).to(self.device)
    ...
    def run(self):
        ...
        next_obs = self.primitive.reset_env()
        ...
        for update in range(1, num_updates + 1):
            ...
            for step in range(0, self.agent_cfg["num_steps"]):
                ...
                next_obs, self.rewards[step], next_done, episodeLen, episodeRet = (
                    self.primitive.env.obs_buf.clone(),
                    self.primitive.env.reward_buf,
                    self.primitive.env.reset_buf.clone(),
                    self.primitive.env.progress_buf.clone(),
                    self.primitive.env.return_buf.clone(),
                )
                self.primitive.env.reset()

                r = self.primitive.calc_reward(next_obs, self.primitive.w_eval)
            ...
            # flatten the batch
            b_obs = self.obs.reshape((-1, self.primitive.env.num_obs))
            b_logprobs = self.logprobs.reshape(-1)
            b_actions = self.actions.reshape((-1, self.primitive.feature_dim))
